...
export default function CheckIn() {
  ...
  async function loadCheckins(nextPage = page) {
    try {
      ...
      setLoadList(false);
      setTotal(response.data.count);
      setTotalLoad(response.data.length);
      setCheckins(data);
    } catch (err) {
      Alert.alert(
        'Erro ao carregar check-in',
        'Não foi póssivel carregar os check-ins'
      );
    }
  }
  ...
  async function handleCheckin(id) {
    try {
      ...
      Alert.alert('Erro ao realizar check-in', 'Maximo de check-ins na semana');
    }
  }
      ...
      <Content>
        ...
        {loadList ? (
          ...
          <CheckinList
            ...
            renderItem={({ item }) => (
              <Check>
                <CheckNumber>Check-in #{item.id}</CheckNumber>
